...
@section('content')
    ...
    <div class="bg-gradient-to-br from-white via-gray-50 to-white rounded-2xl shadow-md border border-gray-100 p-8 mb-6">
        ...
        <form action="{{ route('pemetaan.form.tabel') }}" method="GET" id="filterForm"
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        ...
                        placeholder="Ketik nama atau NIM..." autocomplete="off" oninput="handleSearchInput(this)"
                        onkeydown="handleSearchKeydown(event)" onfocus="showSearchSuggestions()"
                    ...
                    @if (request('search'))
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                            <span
                                class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-primary/10 text-primary">
                                Aktif
                            </span>
                        </div>
                    @endif
                    ...
                    <div id="searchSuggestions"
                        class="suggestions-dropdown absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg mt-1 hidden z-20 max-h-60 overflow-y-auto">
                ...
                <div class="relative group">
                    ...
                    <input type="text" id="province_search"
                        ...
                        class="w-full px-4 py-3.5 pl-12 bg-white border-2 border-gray-200 rounded-xl text-sm
                               ...
                               transition-all duration-200 placeholder-gray-400
                               hover:border-gray-300 hover:shadow-sm"
                        placeholder="Ketik nama provinsi..." autocomplete="off" oninput="handleProvinceSearch(this)"
                        onkeydown="handleProvinceKeydown(event)" onfocus="showProvinceSuggestions()"
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <input type="text" id="city_search"
                        ...
                        autocomplete="off" {{ !request('province_code') ? 'disabled' : '' }}
                        oninput="handleCitySearch(this)" onkeydown="handleCityKeydown(event)"
                        onfocus="showCitySuggestions()" onblur="hideCitySuggestions()">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <svg class="h-5 w-5 text-gray-400 group-focus-within:text-primary transition-colors"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    <select name="beasiswa" id="beasiswa" onchange="this.form.submit()"
                        ...
                        <option value="beasiswa" {{ request('beasiswa') == 'beasiswa' ? 'selected' : '' }}>Beasiswa
                        </option>
                        <option value="non_beasiswa" {{ request('beasiswa') == 'non_beasiswa' ? 'selected' : '' }}>Non
                            Beasiswa</option>
                    </select>
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <svg class="h-5 w-5 text-gray-400 group-focus-within:text-primary transition-colors"
        ...
        @if (request('search') ||
                request('date') ||
                (request('beasiswa') && request('beasiswa') !== 'all') ||
                request('province_code') ||
                request('city_code'))
            <div class="mt-6 pt-6 border-t border-gray-200">
                <a href="{{ route('pemetaan.form.tabel') }}"
                    class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-red-50 to-red-100 text-red-700 rounded-xl text-sm font-semibold hover:from-red-100 hover:to-red-200 transition-all duration-200 shadow-sm hover:shadow border border-red-200">
        ...
        <script>
            ...
            const provinces = @json(
                $provinces->map(function ($p) {
                    return ['code' => $p->province_code, 'name' => $p->province];
                }));
            const allCities = @json(
                $allCities->map(function ($c) {
                    return ['code' => $c->city_code, 'name' => $c->city, 'province_code' => $c->province_code];
                }));
            let searchData = [];
            let currentFocus = -1;
            ...
            function showSearchSuggestions(query = '') {
                ...
                if (!query) {
                    dropdown.innerHTML =
                        '<div class="p-2 text-sm text-gray-500 text-center">Mulai mengetik untuk mencari...</div>';
                ...
                if (filtered.length === 0) {
                    ...
                    dropdown.innerHTML = filtered.map((item, index) =>
                        `<div class="suggestion-item p-2 cursor-pointer hover:bg-gray-100 border-b border-gray-100 last:border-b-0" 
                          onclick="selectSearchSuggestion('${item.nama}', '${item.nim}')" 
                          data-index="${index}">
                        <div class="font-medium text-sm">${highlightMatch(item.nama, query)}</div>
            ...
            function showProvinceSuggestions(query = '') {
                ...
                if (!query) {
                    dropdown.innerHTML =
                        '<div class="p-2 text-sm text-gray-500 text-center">Mulai mengetik untuk mencari provinsi...</div>';
                    dropdown.classList.remove('hidden');
                    return;
                }
                ...
                if (filtered.length === 0) {
                    dropdown.innerHTML =
                        '<div class="p-2 text-sm text-gray-500 text-center">Tidak ada provinsi ditemukan</div>';
                } else {
            ...
            function showCitySuggestions(query = '', provinceCode = '') {
                ...
                if (!query) {
                    dropdown.innerHTML =
                        '<div class="p-2 text-sm text-gray-500 text-center">Mulai mengetik untuk mencari kota...</div>';
                    dropdown.classList.remove('hidden');
                    return;
                }
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <select id="sortColumn"
                            ...
                            <option value="asal_sekolah" {{ request('sort') == 'asal_sekolah' ? 'selected' : '' }}>Asal
                                Sekolah</option>
                            <option value="tanggal_daftar" {{ request('sort') == 'tanggal_daftar' ? 'selected' : '' }}>
                                Tanggal Daftar</option>
                            <option value="tahu_stih" {{ request('sort') == 'tahu_stih' ? 'selected' : '' }}>Tahu STIH
                                Darimana</option>
                            <option value="jenis_beasiswa" {{ request('sort') == 'jenis_beasiswa' ? 'selected' : '' }}>
                                Jenis Beasiswa</option>
                        </select>
                        <button id="sortDirection" onclick="toggleSort()"
                            ...
                            <i id="sortIcon"
                                class="fas {{ request('direction') == 'desc' ? 'fa-sort-down' : 'fa-sort-up' }}"></i>
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        ...
                                        $school =
                                            $mahasiswa->highSchool ??
                                            ($mahasiswa->madrasahAliyah ?? $mahasiswa->vocationalHighSchool);
                                    ...
                                    @elseif($npsn)
                                        <span class="text-gray-400 text-sm"
                                            title="Sekolah dengan NPSN {{ $npsn }} tidak ditemukan di database">
                                            {{ $npsn }}
                                        </span>
                                    @else
                                ...
                                <td>{{ $mahasiswa->tanggal_daftar ? $mahasiswa->tanggal_daftar->format('d/m/Y') : '-' }}
                                </td>
                                <td>{{ $mahasiswa->tahuStih->sumber ?? '-' }}</td>
                                <td>
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $mahasiswa->sumber_beasiswa == 'beasiswa' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                ...
                                <td>
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('pemetaan.form.edit', $mahasiswa->id) }}"
        ...
        @if ($mahasiswas->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                <div class="text-sm text-gray-700">
                    Menampilkan {{ $mahasiswas->firstItem() }} sampai {{ $mahasiswas->lastItem() }} dari
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                        ...
                        @endphp
                        <a href="{{ route('pemetaan.form.tabel', $prevParams) }}"
                            class="px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">Sebelumnya</a>
                    @endif
                    ...
                    @if ($start > 1)
                        @php
                            ...
                            $firstParams['page'] = 1;
                        @endphp
                        <a href="{{ route('pemetaan.form.tabel', $firstParams) }}"
                            class="px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">1</a>
                        @if ($start > 2)
                    ...
                    @foreach ($mahasiswas->getUrlRange($start, $end) as $page => $url)
                        @if ($page == $mahasiswas->currentPage())
                            <span
                                class="px-3 py-2 text-sm bg-primary text-white rounded-lg font-semibold">{{ $page }}</span>
                        @else
                            @php
                                $pageParams = request()->all();
                                $pageParams['page'] = $page;
                            @endphp
                            <a href="{{ route('pemetaan.form.tabel', $pageParams) }}"
                                class="px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">{{ $page }}</a>
                        @endif
                    @endforeach
                    ...
                    @if ($end < $mahasiswas->lastPage())
                        ...
                        @endphp
                        <a href="{{ route('pemetaan.form.tabel', $lastParams) }}"
                            class="px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">{{ $mahasiswas->lastPage() }}</a>
                    @endif
                    ...
                    @if ($mahasiswas->hasMorePages())
                        ...
                        @endphp
                        <a href="{{ route('pemetaan.form.tabel', $nextParams) }}"
                            class="px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">Selanjutnya</a>
                    @else
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            ...
            <p class="text-sm text-gray-600 text-center mb-6">Apakah Anda yakin ingin menghapus data ini? Tindakan ini
